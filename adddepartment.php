...
$query=sprintf("INSERT INTO departments (fullname, shortname, faculty) VALUES ('%s', '%s','%s')", mysql_real_escape_string($_POST['fullname']),mysql_real_escape_string($_POST['shortname']),mysql_real_escape_string($_GET['id']));
$res = mysql_query($query); 
if (!$res)
{
echo 'HTTP/1.1 500 Internal Server Error';
...
<?php
	 ...
	 echo '<span class="bigtext">Кафедра создана!</span><br>';
	 echo '<a href="department.php?id='.$row[0].'" class="medtext" style="text-decoration:none; color: #303030;">Перейти к кафедре</a>';
...
<span class="bigtext">Добавить кафедру</span><br><br>
<form method="post" action="adddepartment.php?id=<?php echo $_GET['id']; ?>" class="medtext" enctype="multipart/form-data">
