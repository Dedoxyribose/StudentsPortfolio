...
$query="SELECT id, fullname, shortname, univer FROM $userstable WHERE id=".$row['faculty'];
...
echo '<table style="margin-left:5%;"><tr><td width="5%" valign="top"><img src="arrow.png" style="width: 100%;"></td><td width="72%"><div><a href="department.php?id='.$row['id'].'" style="text-decoration:none;"><div class="lightitem">'.$row['shortname'].'</div></a></div></td><td width="20%"><td></tr></table>';
...
<form method="post" id="panel" style="display:none;" action="department.php?id=<?php echo $_GET['id']; ?>" class="medtext" enctype="multipart/form-data">
...
<div class="subtitle"> Список направлений </div> <br/> 
...
echo '<a href="addspeciality.php?id='.$row['id'].'" style="text-decoration:none;"><div class="lightitem" style="text-align:center; width: 240px; margin:auto; cursor: pointer;">Добавить направление</div></a>';
...
$query=sprintf("SELECT id, fullname, shortname FROM $userstable WHERE department='%s'",
mysql_real_escape_string($_GET['id']));
...
{
echo '<a href="speciality.php?id='.$row['id'].'" style="text-decoration:none;"><div class="item">'.$row['fullname'].'</div></a>';
}
}
else
{
...
echo '<br/><iframe id="idIframe" onload="iframeLoaded()" src="students.php?department='.$_GET['id'].'" style="width:120%; border: none;" ></iframe>';
